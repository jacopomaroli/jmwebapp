...
const feature = loadFeature('test/integration/routing3.feature')
...
defineFeature(feature, test => {
  ...
  test('Routing with less than loadMaxArticles(3) articles between two visited articles', ({ given, when, then }) => {
    let div
    let myComponent
    ...
    let threeArticlesRootNewerThanD = mockResponses.ArticleListData.threeArticlesRootNewerThanD
    let articleA = mockResponses.PostContentData.articleA
    let articleD = mockResponses.PostContentData.articleD
    ...
    then('if I open article D', () => {
      link.flushMockedResponses()
      link.addMockedResponse({ request: { query: addTypenameToDocument(ArticleContentQuery), variables: { name: articleD.post.post_name } }, result: { data: articleD } })

      let waitStep = mockedModules['PostContent'].getWaitingPromise('default')

      const myTitle = div.querySelector('#post-' + articleD.post.id + ' .entry-title a')
      Simulate.click(myTitle)
    ...
    then('the browser should render article D for the first time', () => {
      ...
      const paramsArticleD = { data: { loading: null, error: null, posts: [ articleD.post ], baseUrl: '/' }, disabled: false, showLoadNewer: false, showLoadOlder: false }
      paramsArticleD.data.loadNewer = (postData) => ArticlesListUtils.genLoadNewer('/', postData)
      paramsArticleD.data.loadOlder = (postData) => ArticlesListUtils.genLoadOlder('/', postData)
      paramsArticleD.articlesListInstances = [
        {
          wrappedInstance: {
            props: {
              data: paramsArticleD.data
            }
          }
        }
      ]
      paramsArticleD.articlesListComponents = [
        {
          props: {
            disabled: false
          }
      ...
      paramsArticleD.parentInstance = paramsArticleD.articlesListInstances[0].wrappedInstance
      const retArticleD = mockedModules['ArticlesList'].component.default(paramsArticleD, {})

      RetThreeArticlesRoot1.props.children[1][0] = retArticleA.props.children[1][0]
      RetThreeArticlesRoot2.props.children[1][0] = retArticleD.props.children[1][0]
      ...
      const absLink = permalink2abs(articleD.post.permalink)
    ...
    then('the browser should render article D for the second time', () => {
      const paramsArticleD = { data: { loading: null, error: null, posts: [articleD.post], baseUrl: '/' }, disabled: false, showLoadNewer: true, showLoadOlder: true }
      paramsArticleD.data.loadNewer = (postData) => ArticlesListUtils.genLoadNewer('/', postData)
      paramsArticleD.data.loadOlder = (postData) => ArticlesListUtils.genLoadOlder('/', postData)
      paramsArticleD.articlesListInstances = [
        {
      ...
      const assertComponent = <React.Fragment>{ RetArticleD }</React.Fragment>
    ...
    then('the browser should render article A for the second time', () => {
      ...
      const assertComponent = <React.Fragment>{ retArticleA }{ retArticleD }</React.Fragment>
    ...
    then('if I request articles newer than D', () => {
      let dateCursor = new Date(articleD.post.post_date)
      dateCursor.setSeconds(dateCursor.getSeconds() + 1)
      const fromDate = dateCursor.toISOString()
      link.flushMockedResponses()
      link.addMockedResponse({ request: { query: addTypenameToDocument(ArticlesListQuery), variables: { permalink: '/%category%/%year%/%monthnum%/%postname%', postsCategory: 'Root', direction: 'ASC', fromDate } }, result: { data: threeArticlesRootNewerThanD } })

      let waitStep = mockedModules['ArticlesList'].getWaitingPromise('default')

      const loadNewer = div.querySelector('.postWrapper[assetid="post-' + articleD.post.id + '"]').previousElementSibling
    ...
    then('the browser should render articles B and C', () => {
      const paramsThreeArticlesRoot1 = { data: { loading: null, error: null, posts: threeArticlesRoot1.category.posts, baseUrl: '/' }, disabled: false, showLoadNewer: false, showLoadOlder: false }
      paramsThreeArticlesRoot1.data.loadNewer = (postData) => ArticlesListUtils.genLoadNewer('/', postData)
      paramsThreeArticlesRoot1.data.loadOlder = (postData) => ArticlesListUtils.genLoadOlder('/', postData)
      paramsThreeArticlesRoot1.articlesListInstances = [
        {
      ...
      const paramsArticleD = { data: { loading: null, error: null, posts: [ articleD.post ], baseUrl: '/' }, disabled: false, showLoadNewer: false, showLoadOlder: true }
      paramsArticleD.data.loadNewer = (postData) => ArticlesListUtils.genLoadNewer('/', postData)
      paramsArticleD.data.loadOlder = (postData) => ArticlesListUtils.genLoadOlder('/', postData)
      paramsArticleD.articlesListInstances = [
        {
      ...
      const assertComponent = <React.Fragment>{ RetThreeArticlesRoot1 }{ retArticleD }</React.Fragment>
